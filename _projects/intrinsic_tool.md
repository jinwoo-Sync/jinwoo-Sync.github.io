...
- **엔지니어링 역량 입증 및 역할 확장**: 주니어 개발자로서 본 프로젝트에 참여하여 회사 핵심 도메인인 캘리브레이션과 카메라 센서 특성(Intrinsic)에 대한 깊이 있는 연구와 학습을 병행함. 이를 통해 구현 원리를 습득하고 유지보수를 성공적으로 수행하며 팀 내 기술적 신뢰를 확보, 이후 '현대자동차 캘리브레이션 툴 개발' 등 대규모 외주 프로젝트의 리드 개발자로 발탁되는 결정적 계기를 마련
...
- **해결 방안 (Action)**: 기존 오픈소스 MTF 분석 모듈을 캘리브레이션 현장 환경에 적용하는 과정에서, 표준 Michelson Contrast 공식(`(Lmax−Lmin)/(Lmax+Lmin)`)이 체커보드 이진 밝기 구조에서 초점 변화 민감도가 낮다는 점을 파악. 분모를 `Lmax`로 변경하여 흑백 대비 변화에 더 민감하게 반응하도록 수식을 재정의하고, 산출된 MTF 값과 1D 프로파일 곡선을 Qt UI에 실시간 연동.
