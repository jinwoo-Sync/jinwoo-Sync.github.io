...
> **담당 역할: FOV Coverage 시각화 모듈 통합 및 유지보수**

캘리브레이션 데이터의 신뢰성을 확보하기 위해, 체커보드가 카메라의 FOV(Field of View) 전체를 고르게 커버하고 있는지 직관적으로 확인하는 시각화 파이프라인을 관리했습니다. 기존 모듈의 수학적 핵심 로직(Sin/Cos 기반 거리 인코딩)을 정밀하게 분석하여 메인 프로그램 내에 안정적으로 연동되도록 코드를 리팩토링하고 지속적인 유지보수를 수행했습니다.
