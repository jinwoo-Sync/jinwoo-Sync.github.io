...
### 5. Top-View LUT 파이프라인 설계

- **문제 상황/목표**: 런타임마다 카메라 투영 연산을 반복하면 실시간 Bird's-eye View 합성이 불가능. 연산을 사전에 계산해 저장하는 LUT 방식 도입 필요.
- **해결 방안 (Action)**:
  1. 카메라 Intrinsic/Extrinsic, 차량 치수, 마스크 등 입력 파라미터 로딩
  2. Top-View 픽셀을 차량 좌표계 XY 평면에 투영 (`getI2W_TopViewPinhole`)
  3. 각 카메라의 R\|T와 Intrinsic으로 재투영해 원본 이미지 픽셀 (ix, iy) 계산
  4. LUT에 "Top-View (u, v) ↔ 카메라 N의 (ix, iy)" 매핑 저장
  5. 런타임에는 LUT를 그대로 참조해 빠르게 Bird's-eye View 합성
- **픽셀 스케일**:
  - HD SVM은 `TOPVIEWVIRTUALCAM_PIXEL_DENSITY ≈ 203.39 px/m` 상수로 고정 → 1픽셀 ≈ 4.92 mm
  - 다른 SVM 툴은 ROI(전·후·좌·우 길이)와 출력 해상도(예: 1024×1024)로 픽셀당 거리 결정 (예: 10 m / 1024 ≈ 9.8 mm)
- **LUT 재사용 조건**: Intrinsic/Extrinsic, 차량 치수, ROI가 바뀌지 않는 한 LUT를 반복 사용. 파라미터 변경 시에만 재생성.
- **결과 (Result)**: 복잡한 투영 연산을 오프라인으로 사전 계산하여 런타임 부하를 최소화하고, 파라미터 변경 없이는 LUT를 재사용함으로써 안정적인 실시간 탑뷰 합성 파이프라인 구축.
