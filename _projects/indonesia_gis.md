...
> **[참고] C# 델리게이트 = 타입 안전한 함수 포인터**
> C++의 `void (*fn)(...)` 함수 포인터와 개념이 동일하다. C++에서 `void*`로 함수 주소를 넘기고 캐스팅해서 호출하는 것처럼, C#에서는 `delegate` 타입을 선언하고 `Marshal.GetDelegateForFunctionPointer<T>()`로 네이티브 함수 주소를 그 타입으로 변환한다. 차이는 C#이 타입을 컴파일 타임에 검증해준다는 것. `NativeLibrary.GetExport()`로 가져온 함수 주소(포인터)를 C#이 안전하게 호출할 수 있는 형태로 감싸는 래퍼가 델리게이트다.
