...
### 2. 센서 퓨전 및 위치 추정 안정화
- **문제 상황/목표**: 실시간 데이터 전송 기능뿐만 아니라 **카메라(.mp4) 및 IMU/GNSS(.txt) 데이터의 로컬 로깅 기능**을 모두 수행하는 시스템 특성상, GPS 음영 지역을 포함한 전 구간에서 각 센서 데이터 간의 정교한 시각 동기화 및 위치 정합성 확보가 필수적임.
- **해결 방안 (Action)**: 칼만 필터(Kalman Filter) 기반의 위치 관리 로직을 통합하고, GPS 수신 단절 시 발생하는 시간 오차를 보정하기 위해 로컬 타임스탬프를 소급 적용하는 'Backdating' 로직 구현.
- **결과 (Result)**: 로컬 로깅 데이터와 실시간 전송 환경 모두에서 데이터 간의 시간축 정렬 및 위치 추정 정밀도를 안정적으로 유지.

### 3. 아키텍처 설계 및 실시간 데이터 파이프라인 통합
