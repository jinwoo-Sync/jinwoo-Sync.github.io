...
### [데이터 스트리밍 및 서비스 연동 검증]
- **서버 측 타임스탬프 동기화**: 카메라 프레임과 딥러닝 추론 결과(BBox)를 서버에서 실시간 매핑하기 위해, 동일 타임스탬프 데이터의 안정적인 수신 여부를 검증했다.
![](/assets/images/projects/real-time_poi/timestamp_verification.png)
- **실제 서비스 연동**: 사내 서비스 웹 개발팀과 협의된 JSON 데이터 포맷을 적용하여, 실제 웹 인터페이스 상에서 실시간 탐지 결과가 정상적으로 표출되는지 최종 확인했다.
...
- **회사 기여 성과**: 기존 Lite 장비의 생산·운용 파이프라인(3D 외형 제작, 센서 조립/보정, PC 환경 및 원격 세팅 등)을 앱 설치 및 업데이트 방식으로 일원화하여, **제품 생애 주기 전반의 Man-Month를 혁신적으로 절감할 수 있는 양산형 모바일 솔루션의 기술적 토대**를 마련했다.
