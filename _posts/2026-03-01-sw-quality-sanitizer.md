...
실제 프로젝트(loggingcoremodule, visualizer_cpu)에서 ASAN·TSan·UBSan·clang-tidy를 적용하여 발견하고 수정한 버그 사례를 정리한다.

포인트는 **도구 없이는 재현이 어렵거나, x86에서는 잠자다가 ARM(Jetson)에서 터진 버그들**이다.
...
## 3-2. UBSan — Eigen 메모리 정렬 위반 (ARM 포팅)
