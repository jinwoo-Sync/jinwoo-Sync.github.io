...
파라미터 비중은 아키텍처 구조에서 바로 계산할 수 있다. Transformer 블록 하나에서 Attention은 Wq, Wk, Wv, Wo = `4d²`개, MLP는 W1 + W2 = `8d²`개의 파라미터를 가진다. 코드만 봐도 MLP가 정확히 2배 많다. Attention이 주목받지만, 실제로 모델의 "지식"은 대부분 MLP에 저장되어 있다 (Geva et al., EMNLP 2021; Meng et al., NeurIPS 2022).

---

## 6. 둘이 합쳐지면: Transformer 블록의 시너지
